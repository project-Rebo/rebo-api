...
public class ReportService {
    ...
    public ReportResponse generateCategoryReport(CategorySearchRequest request) {
        ...
        // 1. 매장 데이터 분석
        StoreAnalysisCache analysisData = storeService.generateCategory(request);
    ...
    public ReportResponse saveReportWithAllData(Long memberId, ReportAllSaveRequest request) {
        try {
            ...
            // 1. StoreInput 저장
            StoreInput storeInput = storeInputService.saveStoreInput(memberId, request.getStoreInputData());
            log.info("StoreInput 저장 완료 - id: {}", storeInput.getId());
            
            // 2. Region 저장 (StoreInput.id를 storeInputId로 사용)
            Region region = regionService.saveRegion(storeInput.getId(), request.getRegionData());
    ...
    public List<Report> getMemberReports(Long memberId) {
        log.info("회원별 리포트 목록 조회 - memberId: {}", memberId);
